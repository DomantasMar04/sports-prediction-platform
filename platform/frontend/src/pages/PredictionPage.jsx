...
function CountdownDisplay({ remaining }) {
    ...
    if (remaining <= 0) return <Badge bg="danger" className="fs-6 p-2">🔴 Prasidėjo — uždaryta</Badge>;
...
// --- TAŠKŲ DETALIZACIJA (RODO VISAS REIKŠMES) ---
function BreakdownCard({ breakdown }) {
    // 1. Patikriname ar išvis turime duomenis
    if (!breakdown) return <Alert variant="warning" className="mt-3">Išklotinės duomenų nėra</Alert>;

    let data;
    try {
        // 2. Saugus JSON parsinimas
        data = typeof breakdown === 'string' ? JSON.parse(breakdown) : breakdown;
    } catch (e) {
        return <Alert variant="danger" className="mt-3">Klaida skaitant taškų formatą</Alert>;
    }
    ...
    // 3. Konvertuojame į masyvą rodymui
    const rows = Object.entries(data).map(([key, value]) => ({
        label: labels[key] || `Kategorija: ${key}`, // Jei neras vertimo, parodys originalų raktą iš API
        points: value ?? 0
    }));

    // 4. Suskaičiuojame sumą patikrinimui
    const calculatedTotal = rows.reduce((acc, curr) => acc + curr.points, 0);

    return (
        <Card className="mt-3 border-success shadow-sm">
            <Card.Body>
                <Card.Title className="fs-6 text-success fw-bold border-bottom pb-2">
                    Taškų skaičiavimas (Detaliai)
                </Card.Title>
                <table className="table table-sm mb-0">
                    ...
                    {rows.length > 0 ? (
                        ...
                        <tr><td colSpan="2" className="text-center text-danger">Duomenų masyvas tuščias</td></tr>
                    )}
                    <tr className="table-success fw-bold border-top">
                        <td>Suma iš detalių</td>
                        <td className="text-end">{calculatedTotal} taškų</td>
                    </tr>
                    </tbody>
                </table>
            </Card.Body>
            ...
            <Container className="mt-4" style={{ maxWidth: '680px' }}>
                ...
                <Card className="mb-3 shadow-sm text-center">
                    <Card.Body>
                        ...
                        {isFinished && (
                            <div className="mt-2">
                                <span className="text-muted">Rezultatas:</span>
                                <h3 className="text-primary">{match.homeScore} : {match.awayScore}</h3>
                            </div>
                        )}
                        {!isFinished && !isLive && <CountdownDisplay remaining={remaining} />}
